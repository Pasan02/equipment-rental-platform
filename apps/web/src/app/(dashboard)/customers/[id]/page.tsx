    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="border-slate-200 bg-white text-slate-900 shadow-sm p-5">
            <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2 font-heading">
              <Calendar className="h-4 w-4 text-blue-600" /> Reservation History
            </h3>
            {!customer.customerReservations || customer.customerReservations.length === 0 ? (
              <p className="text-xs text-slate-400 py-6 text-center">No reservations made by this customer yet.</p>
            ) : (
              <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl bg-slate-50/50 overflow-hidden text-xs">
                {customer.customerReservations.map((res) => (
                  <div key={res.id} className="p-3.5 flex items-center justify-between hover:bg-slate-100/50 transition-colors">
                    <div>
                      <Link
        ...
        <div className="space-y-6">
          <Card className="border-slate-200 bg-white text-slate-900 shadow-sm p-5">
            <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2 font-heading">
              <FileText className="h-4 w-4 text-purple-600" /> Verification Documents
            </h3>
            {!customer.uploads || customer.uploads.length === 0 ? (
              <p className="text-xs text-slate-400 py-6 text-center">No identity/agreement documents uploaded.</p>
            ) : (
                ...
                {customer.uploads.map((doc) => (
                  ...
                  >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-blue-600 hover:text-blue-700 p-1.5 cursor-pointer"
